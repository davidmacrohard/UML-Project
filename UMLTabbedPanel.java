...
/**
 * Handles the tabbed panels of the program. User can create new tabs on which
 * to work, so that multiple canvases can be manipulated simultaneously, but
 * separately.
 * 
...
public class UMLTabbedPanel extends JTabbedPane implements ActionListener {
	...
	private static Icon CLOSE_TAB_ICON_NORMAL = null;
	private static Icon CLOSE_TAB_ICON_HOVER = null;
	private static Icon ADD_TAB_ICON_NORMAL = null;
	private static Icon ADD_TAB_ICON_HOVER = null;

	UMLTabbedPanel(UMLToolBar toolBar) {
		// Save a reference to the toolbar as the umlCanvas uses it
		umlToolBar = toolBar;

		CLOSE_TAB_ICON_NORMAL = new ImageIcon(this.getClass().getResource(
				"images/file_icon_normal.png"));
		CLOSE_TAB_ICON_HOVER = new ImageIcon(this.getClass().getResource(
				"images/file_icon_hover.png"));
		ADD_TAB_ICON_NORMAL = new ImageIcon(this.getClass().getResource(
				"images/add_icon_normal.png"));
		ADD_TAB_ICON_HOVER = new ImageIcon(this.getClass().getResource(
				"images/add_icon_hover.png"));
			...
			if (c instanceof UMLCanvas) {
				...
				for (Component c2 : ((UMLCanvas) c).getComponents()) {
					...
					if (c2 instanceof UMLLine) {
						// Cast the c2 component that is a line into a UMLLine
						// object
						UMLLine line = ((UMLLine) c2);

						// If it's successful (just to be safe)
						if (line != null) {
							// Resize the line object so it fits the new
							// dimensions
